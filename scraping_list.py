...
def get_airline_tables(airline_tuple):
    ...
    try:
        ...
        CaptMax, CaptMin = list_tables[3].iloc[2:4,1].tolist()
        DateCaptMax, DateCaptMin = list_tables[3].iloc[2:4,2].tolist()
        return airline_url, airline_name, CaptMax, CaptMin, last_date, country, DateCaptMax, DateCaptMin
    except Exception as e:
            return e, airline_url, airline_name
...
def clean_salaries(salaries):
    df_salary = pd.DataFrame([salary for salary in salaries if len(salary) == 8])
    df_salary.columns = ['URL', 'AirlineName', 'CaptMax', 'CaptMin', 'LastUpdate', 'Country', 'DateCaptMax', 'DateCaptMin']
    
    # DF Stack on Top/Base. Requirement for ML model (Olivier)
    df_stack = (df_salary.set_index(['URL', 'AirlineName', 'LastUpdate', 'Country', 'DateCaptMax', 'DateCaptMin'])
        .rename_axis(['Top/Base'], axis=1)
        .stack(dropna=False) # Put True to remove NaNs
        .reset_index())
    df_stack.columns = ['URL', 'AirlineName', 'LastUpdate', 'Country', 'DateCaptMax', 'DateCaptMin', 'Top/Base', 'Salary']
    ...
    clean_order = ['URL', 'Country', 'AirlineName', 'Year', 'Top/Base', 'Salary']
...
if __name__ == "__main__":
    try: 
        with open("squawkit_airline_jobs.json", "r") as fd:
    ...
    with open("squawkit_airline_jobs.json", "w") as fd:
        json.dump(jobs, fd)